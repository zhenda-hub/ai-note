...
要提升成功率，关键是“工程化”AI的使用，将其转化为可控、可迭代的系统

单步准确率是90%, 完成一步测试一步，有异常仅限修改一次， 修改的准确率也是90%。 一共有10步，求总体准确率：
...
- 让 AI 扮演不同的角色进行协作，让模型对抗，而不是合作。自动化测试 (TDD)： 采用测试驱动开发。先让 AI 写测试用例，再写功能代码。公式修正： 如果每一步都有 100% 准确的测试拦截错误，那么整体成功率将不再受幂律分布影响，而是取决于你纠错的次数。
  - 规划者 (Planner)： 负责拆解需求，输出技术设计文档和任务列表。
  - 执行者 (Coder)： 每次只负责实现一个小模块。
  - 审阅者 (Reviewer)： 负责检查代码是否符合最初的设计规范，发现 Bug。
  - 测试者 (Tester)： 编写单元测试，验证执行结果。
- 对复杂项目拆分子功能，子功能可以继续拆分
- 先plan再act
- 总结当前的项目进度和已确定的技术决策方案文档，技术栈和未来扩展方向
