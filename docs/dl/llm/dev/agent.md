...
关键点:多 agent 不是"人多力量大"自动就好。它真正的成本和技巧在两个单点环节——

1. 拆分 + 定契约(开工前):必须把活切成互不依赖的块,并把接口钉死。我这次让 4 个文件只依赖 three 和 config.js,谁都不 import 谁,所以才能真并行。切不干净,并行就变成互相踩脚。
2. 集成(收工后):并行产出回来后,得有人(main.js)把它们缝起来并验证。这步无法并行。

拆得好,并行才有意义;拆不好,并行变成互相等、互相冲突
